...
module.exports.onAPI = async (req, res) => {
  ...
  if (!text || !author) {
    return res.json({ status: false, error: 'Missing required query: text and author are required.' })
  }

  if (color && !['black', 'white'].includes(color.toLowerCase())) {
    return res.json({ status: false, error: 'Color must be either "black" or "white".' })
  }

  const words = text.trim().split(' ')
  if (words.length > 25) {
    return res.json({ status: false, error: 'Quote must be 25 words or fewer.' })
  }
  ...
  try {
    ...
    console.error(err)
    res.json({ status: false, error: 'Failed to generate image.' })
  }
}
