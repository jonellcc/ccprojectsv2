...
module.exports.onAPI = async (req, res) => {
  ...
  try {
      ...
      for (const line of lines) {
        if (line.startsWith('data: {"text":')) {
          try {
            ...
            if (parsedData.text) {
              fullTextResponse += parsedData.text;
            }
          } catch (e) {
    ...
    apiResponse.data.on('end', () => {
      ...
      res.json({
        response: fullTextResponse.trim() || "Failed to receive a text response from the Quillbot stream."
      });
    });

    apiResponse.data.on('error', (err) => {
      console.error('Stream error:', err.message);
      res.status(500).json({ error: 'Stream error during API call' });
    });
    ...
    if (error.response) {
      ...
      console.error('Quillbot API Error Status:', error.response.status);
      errorMessage = `Quillbot API responded with status ${error.response.status}. The required session headers (Cookie/useridtoken) are likely invalid or expired.`;
    } else {
      console.error('Request Error:', error.message);
      errorMessage = 'Request to Quillbot API failed.';
    }
